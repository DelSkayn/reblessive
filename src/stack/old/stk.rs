        ...
        unsafe {
            if let Some(x) = this.f.take() {
                ...
                with_stack_context(|stack| {
                    ...
                    assert_eq!(stack.state.get(), State::Base, "Tried to push a task while another stack future is running. Stack futures need be completed before another task can be ran.");
                    ...
                    if stack.is_root_context(cx) {
                        stack.push_new_task(async move {
            ...
            if let Some(x) = (*this.res.get()).take() {
                ...
                if this.running {
                    with_stack_context(|stack| assert_eq!(stack.state.get(), State::NewTask))
                }
            }
        }
        Poll::Pending
...
impl<'a, F, R, M> Drop for InnerStkFuture<'a, F, R, M> {
    fn drop(&mut self) {
        if self.running && self.res.get_mut().is_none() {
            ...
            with_stack_context(|stack| {
                if stack.state.get() != State::Cancelled {
                    stack.pop_cancelled_task()
                }
            })
        }
    }
